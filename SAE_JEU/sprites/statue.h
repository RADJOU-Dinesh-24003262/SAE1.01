...
void Statue(MinGL &window, int Center1, int Center2, int mat_x, int mat_y){
    ...
    float body_y          = 5 + Center2;
    float foot_size_x     = body_size/2;
    float foot_size_y     = body_size/8;
    float foot1_x         = Center1 - 5;
    float foot2_x         = Center1 + 5;
    float foot_y          = 5 + Center2 + 9;
    float TopSocle1_x     = Center1 - body_size;
    float TopSocle1_y     = 5 + Center2 + 10;
    float TopSocle2_x     = Center1 + body_size;
    float TopSocle2_y     = 5 + Center2 + 11;
    float DownSocle1_x    = TopSocle1_x - 3;
    float DownSocle1_y    = TopSocle2_y;
    float DownSocle2_x    = TopSocle2_x + 3;
    float DownSocle2_y    = TopSocle2_y + 5;
    float Hand_size_x     = body_size/3;
    float Hand_size_y     = body_size/2;
    float Hand1_x         = TopSocle1_x;
    float Hand2_x         = TopSocle2_x;
    float Hand_y          = 5 + Center2 - 5;
    float Shoulder1_x     = Center1 - body_size;
    float Shoulder1_y     = 5 + Center2 - body_size;
    float Shoulder2_x     = Center1 + body_size;
    float Shoulder2_y     = 5 + Center2 - (body_size/2);
    float LeftBretelle1_x = Center1 - (body_size/2) - 1;
    float RightBretelle1_x= Center1 + (body_size/2) - 1;
    float Bretelle1_y     = Shoulder1_y;
    float LeftBretelle2_x = Center1 - (body_size/2) + 1;
    float RightBretelle2_x= Center1 + (body_size/2) + 1;
